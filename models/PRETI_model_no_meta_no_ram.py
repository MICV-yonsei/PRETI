...
class SiameseMAE(MAEViT):
    ...
    def forward_encoder_no_masking(self, x):
        ...
        # # Extract age_emb and gender_emb from x
        # age_emb_out = x[:, 1:2, :]  # 첫 번째 추가된 age_emb 위치
        # gender_emb_out = x[:, 2:3, :]  # 두 번째 추가된 gender_emb 위치
    ...
    # def forward(self, imgs, roi_masks, inital_mask_ratio, final_mask_ratio, max_epochs, epoch):
    def forward(self, imgs, mask_ratio, max_epochs, epoch):
        ...
        # masked_latent, masked_mask, masked_ids_restore = self.forward_encoder(masked_img, mask_ratio, masked_roi_mask)
        ...
        return total_loss, reconstruct_loss, feature_sim_loss, perceptual_loss, [masked_pred], [masked_mask]
